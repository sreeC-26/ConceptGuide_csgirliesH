...
export default function GoalCard({ goal, onToggle, onDelete }) {
  ...
  return (
    <div 
      className={`relative rounded-xl p-3 sm:p-5 border transition-all duration-300 ${
        goal.isActive 
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-3 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="text-2xl sm:text-3xl">{goalTypeIcons[goal.type] || '🎯'}</span>
          <div>
            <h3 className="font-bold text-white text-sm sm:text-lg" style={{ fontFamily: 'Poppins, sans-serif' }}>
              {goal.name}
            </h3>
      ...
      <div className="mb-3 sm:mb-4">
        <div className="flex justify-between items-end mb-2">
          <div>
            <span className="text-xl sm:text-3xl font-bold text-white">{progress?.current || 0}</span>
            <span className="text-gray-400 text-sm sm:text-lg"> / {progress?.target || goal.target}</span>
          </div>
          <span className={`text-lg sm:text-2xl font-bold ${isCompleted ? 'text-green-400' : 'text-pink-400'}`}>
            {percentage}%
          </span>
        </div>
      ...
      {/* Actions */}
      <div className="flex items-center gap-2 pt-2 sm:pt-3 border-t border-gray-700">
        <button
          onClick={onToggle}
